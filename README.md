...
```bash
pip install "git+https://github.com/BayyinahEnterprise/furqan-lint.git@v0.11.6"
```

Replace `v0.11.6` with any tag from the [release history](https://github.com/BayyinahEnterprise/furqan-lint/releases) or `main` for the development tip.

### Furqan dependency

furqan-lint requires `furqan>=0.11.0`, the Furqan programming-language tooling. As of 2026-05-03 the PyPI release of `furqan` is at v0.10.1; install v0.11.6 directly from GitHub:

```bash
pip install "git+https://github.com/BayyinahEnterprise/furqan-programming-language.git@v0.11.6"
```

This GitHub-pin step will not be necessary once `furqan` v0.11.6 is published to PyPI.
...
jobs:
  lint:
    ...
    steps:
      ...
      - uses: BayyinahEnterprise/furqan-lint@v0.11.6
        with:
          path: src/
```
...
was retired in v0.11.6 (Phase G10.5 al-Mubin) because
