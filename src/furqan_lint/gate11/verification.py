...
class Verifier:
    ...
    def step6_verify_sigstore(
        ...
        CASM-V-030..034 mapped from sigstore VerificationError.
        """
        try:
        ...
        # sigstore-python 3.x exposes Verifier.production() / .staging()
        # which wire the rekor client and trusted root automatically.
        verifier = (
        ...
        # Extract OIDC identity from the certificate.
        try:
            cert = sg_bundle.signing_certificate
            for ext in cert.extensions:
                # Subject Alternative Name is the canonical OIDC
                # identity carrier in Fulcio-issued certificates.
                if ext.oid.dotted_string == "2.5.29.17":
                    return str(ext.value)
            return "<unknown OIDC identity>"
        except Exception:
            return "<unknown OIDC identity>"
    ...
    def verify_bundle(
        ...
        signed_by = self.step6_verify_sigstore(bundle, canonical, trusted_root)
