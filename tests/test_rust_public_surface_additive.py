...
def test_rust_adapter_public_surface_is_superset_of_v0_8_5_baseline() -> None:
    """The v0.8.5 baseline (alias of v0_8_4; the v0.8.5
    attribution-corrective is a documentation-only release
    that sweeps source-code inline comments but does not
    change the rust_adapter __all__ surface). Removals
    require a major-version bump.
    """
    from furqan_lint import rust_adapter

    current = frozenset(rust_adapter.__all__)
    missing = _RUST_ADAPTER_PUBLIC_SURFACE_v0_8_5 - current
    assert not missing, (
        f"rust_adapter.__all__ removed names from v0.8.5 baseline: "
        f"{sorted(missing)}. Removals require a major-version bump."
    )
